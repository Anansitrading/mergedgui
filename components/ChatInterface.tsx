...
export function ChatInterface({ mode = 'full', className }: ChatInterfaceProps) {
  ...
  const handleSend = async () => {
    ...
    const newMessage: Message = { role: 'user', content: inputValue, attachments: [...attachments] };
    ...
    try {
        ...
        setMessages(prev => [...prev, { role: 'model', content: "Error communicating with agent." }]);
    } finally {
        setIsTyping(false);
    }
  };
    ...
    <div className={cn("flex flex-col h-full bg-transparent", className)}>
      ...
      <div className={cn(mode === 'minimal' ? "p-0" : "p-6 pt-2")}>
        {attachments.length > 0 && (
            <div className="flex gap-2 mb-2 overflow-x-auto pb-2 px-1">
                {attachments.map((file, i) => (
                    <div key={i} className="flex items-center gap-2 bg-card border border-border rounded-lg px-3 py-2 text-xs shadow-sm animate-in fade-in slide-in-from-bottom-2">
                        {file.type.startsWith('image') ? <ImageIcon size={14} className="text-chart-4"/> : <FileText size={14} className="text-chart-1"/>}
        ...
        <div className="relative group">
          <div className="absolute inset-0 bg-gradient-to-r from-primary/20 to-chart-3/20 rounded-xl blur-lg opacity-0 group-focus-within:opacity-100 transition-opacity duration-500 pointer-events-none" />
          <div className="relative flex items-end gap-2 bg-card/80 backdrop-blur-xl border border-border rounded-xl p-2 shadow-2xl">
            ...
            <button 
                ...
                className="p-3 text-muted-foreground hover:text-foreground hover:bg-secondary rounded-lg transition-all"
                title="Attach files"
            >
                <Paperclip size={20} />
            </button>
            ...
            <button
              ...
              className={cn(
                ...
                isRecording 
                    ? "bg-destructive text-destructive-foreground animate-pulse shadow-[0_0_15px_var(--destructive)]" 
            ...
            <button
              ...
              className="p-3 bg-primary text-primary-foreground rounded-lg hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md"
            >
              <Send size={20} />
            </button>
          </div>
